...
namespace raphEngine::objects
{
    ...
    std::vector<Texture> loadMaterialTextures(aiMaterial* mat, aiTextureType type, Texture::TextureType typeName, bool filter)
    {
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            bool skip = false;
            for (unsigned int j = 0; j < ObjectMesh::textures_loaded_.size(); j++)
            {
                if (std::strcmp(ObjectMesh::textures_loaded_[j].path.data(), str.C_Str()) == 0)
                {
            ...
            {   // if texture hasn't been loaded already, load it
                ...
                ObjectMesh::textures_loaded_.push_back(texture);  // store it as texture loaded for entire model, to ensure we won't unnecessary load duplicate textures.
            }
        }
        return textures;
    }
    
    std::unique_ptr<Mesh> processMesh(aiMesh* mesh, const aiScene* scene, bool filter, const glm::mat4& ModelMat)
    {
        ...
        std::vector<Texture> diffuseMaps = loadMaterialTextures(material, aiTextureType_DIFFUSE, Texture::DIFFUSE, filter);
        textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());

        std::vector<Texture> specularMaps = loadMaterialTextures(material, aiTextureType_SPECULAR, Texture::SPECULAR, filter);
        textures.insert(textures.end(), specularMaps.begin(), specularMaps.end());

        std::vector<Texture> normalMaps = loadMaterialTextures(material, aiTextureType_NORMALS, Texture::NORMAL, filter);
        textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());

        std::vector<Texture> heightMaps = loadMaterialTextures(material, aiTextureType_HEIGHT, Texture::HEIGHT, filter);
    ...
    void processNode(ObjectMesh* object_mesh, aiNode* node, const aiScene* scene, bool filter)
    {
    ...
    void loadModel(ObjectMesh* object_mesh, std::string const& path, bool filter)
    {
