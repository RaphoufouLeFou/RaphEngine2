...
float ShadowCalculation(vec3 fragPosWorldSpace, vec3 normal)
{
    ...
    {
        bias *= 0.008; // 1 / ((farPlane / cascadePlaneDistances[layer]) *
                       // biasModifier);
    }
...
void main()
{
    ...
    if (fs_in.TangentFragPos.x > 0.5 && fs_in.TangentFragPos.y < 0.5
        || fs_in.TangentFragPos.x < 0.5 && fs_in.TangentFragPos.y > 0.5)
        color = vec3(0, 0, 0);
